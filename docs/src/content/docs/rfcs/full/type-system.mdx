...
use named arguments. This replaces the earlier positional-only design to
ensure one consistent calling convention everywhere. Names are required once
the callee declares two or more parameters or fields; a callee that declares
exactly one may take a lone positional (the arity-1 amendment in
protocols-rfc.md). The "no exceptions" and "maximum 3 parameters" wording below
is the original design record and no longer describes the implemented rule.
