...
- Reads as "pass it up"
- Propagation is an ordinary early return — `!` appends nothing to the
  trace. The complete trace was already captured once, at `throw` (see
  section 5)
...
Errors carry a **captured stack trace**: a native frame-pointer walk taken
**once**, at `throw`, and symbolized through compiler-emitted tables. Zero
steady-state cost — no per-call bookkeeping, the price is paid only when an
error is actually thrown.

### Why the frame-pointer walk, not an incremental append

Aster does not unwind. `!` is "if error, return it" — the stack unwinds by
ordinary returns. That has one decisive consequence: a `throw` executes at
the deepest live frame with the **native stack still fully intact**, so the
whole trace is walkable right there, exactly once. There is no need to
append a frame on every `!` hop and no second source of truth to keep in
sync with the native stack.

### Mechanism

1. Frame pointers are forced on everywhere: Cranelift `preserve_frame_pointers`
   in both the JIT and AOT ISA configs, and `force-frame-pointers` for the
   codegen staticlib so runtime (Rust) frames walk cleanly too.
2. At `throw` (`aster_error_set_typed`), the runtime walks the frame-pointer
   chain from the current frame up to the recorded stack bounds, collecting
   return-address PCs. The main thread, the blocking pool, and green threads
   (mmap'd stacks) all record their stack bounds so the walk stays in bounds
   and never reads past the stack.
3. Capture is **once per error object**. An error that already carries a
   trace keeps it — a `catch` arm that rethrows the same error preserves the
   original trace; constructing and throwing a new error captures fresh.
4. PCs are symbolized through the compiler's own tables (no DWARF, no
   `dladdr`, so binaries stay self-contained):
   - **JIT**: as functions finalize, an address-range → function-id table is
     registered.
   - **AOT**: codegen emits a data section of (function address, size, name)
     entries plus a string table; runtime init walks it into the same sorted
     range structure the JIT uses.
   - **Line info**: FIR lowering threads AST spans through to Cranelift
     srclocs; each finalized function keeps a PC-offset → span table, so a PC
     resolves to file and line.
5. Frames that resolve in the Aster function table become real frames;
   contiguous runtime-internal frames that do not resolve collapse into a
   single `[runtime]` marker rather than noise.
6. `.trace()` is available on any error via `Exception` and returns
   `List[Frame]`, ordered worst frame (deepest / throw site) first. A `Frame`
   is a structured value (`function: String`, `file: String`, `line: Int`,
   `Printable`) — there is no string-only interim API.

### Cost model

- **Happy path**: zero cost. No trace is created, no allocations, no
  bookkeeping on any `!` hop.
- **Throw path**: one frame-pointer walk, bounded by the stack, paid once.
- **`.trace()` call**: materializes the captured PCs into `List[Frame]`,
  symbolizing through the tables already built at compile time.
...
7. **`!`** for error propagation — an ordinary early return, appends nothing to the trace, requires compatible `throws` on caller
...
12. **Errors carry a captured stack trace** — a native frame-pointer walk taken once at `throw`, symbolized through compiler-emitted tables; `.trace() -> List[Frame]`, zero cost on the happy path
...
- **Incremental stacktrace append** — appending a file/line on every `!`
  hop, with a `.stacktrace() -> List[String]` string surface. Rejected: it
  puts steady-state cost on the propagation path to subsidize the rare throw
  and keeps a second source of truth for what the native stack already knows.
  Replaced by a single frame-pointer walk at `throw` and a structured
  `.trace() -> List[Frame]` surface (section 5).
