...
/// Uses checked multiplication to abort on overflow, consistent with
/// aster_int_add/sub/mul. This is an interim measure until BigInt
/// promotion is implemented (see bigint-rfc.md).
#[unsafe(no_mangle)]
    ...
    while e > 0 {
        if e & 1 == 1 {
            result = match result.checked_mul(b) {
                Some(v) => v,
                None => {
                    eprintln!("integer overflow: {} ** {} exceeds 64-bit range", base, exp);
                    std::process::abort();
                }
            };
        }
        if e > 1 {
            b = match b.checked_mul(b) {
                Some(v) => v,
                None => {
                    // Only abort if we'll actually use this squared value.
                    // If the remaining exponent bits would multiply it in,
                    // overflow is real. Otherwise, the squared value is unused.
                    if (e >> 1) > 0 {
                        eprintln!("integer overflow: {} ** {} exceeds 64-bit range", base, exp);
                        std::process::abort();
                    }
                    0 // unreachable, but satisfies the compiler
                }
            };
        }
        e >>= 1;
