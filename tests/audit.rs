...
fn audit_f3_throws_call_without_bang_error() {
    let err = common::check_err(r#"class AppError extends Error
  code: Int
...
fn audit_f3b_throws_call_with_bang_ok() {
    common::check_ok(r#"class AppError extends Error
  code: Int
...
def caller() throws AppError -> Int
  risky()!.catch
    ...
    _ -> -1
"#);
    assert!(err.contains("DatabaseError") || err.contains("subtype") || err.contains("unreachable") || err.contains("not a subtype"));
}
...
fn audit_f9_literal_match_on_nullable() {
    common::check_ok(r#"def check(x: String?) -> Int
  match x
    "hello" => 1
    nil => 0
    _ => -1
...
fn audit_f12_map_nullable_parse() {
    common::check_ok(r#"def get_map() -> Map[String, Int]?
  nil
"#);
}
...
fn audit_f13_task_nullable_parse() {
    common::check_ok(r#"def get_task() -> Task[Int]?
  nil
"#);
}
...
fn audit_h1_unary_recursion_depth_limit() {
    let result = std::thread::Builder::new()
        ...
        .unwrap()
        .join();
    assert!(result.is_ok(), "Thread panicked — stack overflow instead of depth error");
}
...
fn audit_h1b_block_recursion_depth_limit() {
    let result = std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            let mut parser = parser::Parser::new(tokens);
            let result = parser.parse_module("test");
            assert!(result.is_err(), "Expected recursion depth error for nested blocks");
        })
        .unwrap()
        .join();
    assert!(result.is_ok(), "Thread panicked — stack overflow instead of depth error");
...
fn audit_h2_cyclic_extends_detection() {
    ...
    let err = common::check_err(r#"class A extends A
  message: String
"#);
    assert!(err.contains("ircular") || err.contains("ycle"), "Expected cycle error, got: {}", err);
}

// Field shadowing: child class cannot redeclare inherited field
#[test]
fn audit_field_shadowing_in_extends() {
    let err = common::check_err(r#"class Base
  message: String
...
class Child extends Base
  ...
  extra: Int
"#);
    assert!(err.contains("shadow") || err.contains("redeclar") || err.contains("already"),
        "Expected field shadowing error, got: {}", err);
}
...
fn audit_m5_string_length_limit() {
    ...
    assert!(result.is_err(), "Very long string literal should be rejected");
}
...
fn audit_m6_digit_limit() {
    ...
    assert!(result.is_err(), "Very long number literal should be rejected");
}
...
fn audit_d3_generic_class_params_parsed() {
    common::check_ok(r#"class Pair[A, B]
  first: A
  second: B
"#);
}
...
fn audit_d3_generic_function_params_parsed() {
    common::check_ok(r#"def identity[T](x: T) -> T
  x
"#);
}
...
fn audit_d5_if_body_condition_checked() {
    let err = common::check_err(r#"if 42
  let x = 1
"#);
    assert!(err.contains("Bool"), "Expected Bool condition error, got: {}", err);
}

#[test]
fn audit_d5_while_body_checked() {
    common::check_ok(r#"let x = 0
while x < 10
  x = x + 1
"#);
}
...
fn audit_d5_for_body_checked() {
    common::check_ok(r#"let items: List[Int] = [1, 2, 3]
for item in items
  let y = item + 1
"#);
}
...
fn audit_d6_nullable_or_else_returns_inner_type() {
    common::check_ok(r#"let x: Int? = nil
...
fn audit_d6_nullable_or_type_mismatch() {
    let err = common::check_err(r#"let x: Int? = 5
let y: Int = x.or("not_int")
"#);
    assert!(err.contains("mismatch"), "Expected type mismatch, got: {}", err);
}
...
fn audit_d8_match_as_statement() {
    common::check_ok(r#"let x = match 1
  1 => "one"
  _ => "other"
"#);
}
...
fn audit_q5_nullable_method_call_rejected() {
    let err = common::check_err(r#"class Foo
  name: String

let x: Foo? = nil
let y = x.name
"#);
    assert!(err.contains("nullable") || err.contains("Resolve") || err.contains("Cannot access"),
        "Expected nullable access error, got: {}", err);
}
