...
// ─── Helpers for running multi-file programs via the CLI ────────────

/// Write multiple .aster files to a temp directory and run the main file via `asterc run`.
fn run_multi_file(main_src: &str, files: HashMap<&str, &str>) -> std::process::Output {
    let dir = crate::common::make_temp_dir("modules");
    for (path, src) in &files {
        let file_path = dir.join(format!("{}.aster", path));
    ...
    let main_path = dir.join("main.aster");
    {
    ...
    crate::common::cli(&["run", &main_str])
}

// ─── Cross-module method compilation (runtime tests) ────────────────
...
fn import_pub_function_from_module() {
    ...
    crate::common::check_ok_with_files("use utils { double }\nlet x = double(n: 4)\n", files);
}
...
fn import_pub_enum_from_module() {
    ...
    crate::common::check_ok_with_files("use types/color { Color }\nlet c = Color.Red\n", files);
}
...
fn import_pub_let_binding_from_module() {
    ...
    crate::common::check_ok_with_files("use config { VERSION }\nlet v: Int = VERSION\n", files);
}
...
fn selective_import_only_imports_named_items() {
    ...
    let err = crate::common::check_err_with_files("use stuff { Foo }\nlet b = Bar(y: 1)\n", files);
...
fn wildcard_import_imports_all_pub_names() {
    ...
    crate::common::check_ok_with_files("use stuff\nlet f = Foo(x: 1)\nlet b = Bar(y: 2)\n", files);
}
...
fn private_class_not_importable() {
    ...
    let err = crate::common::check_err_with_files("use secret { Hidden }\n", files);
    assert!(
...
fn private_function_not_importable() {
    ...
    let err = crate::common::check_err_with_files("use secret { internal }\n", files);
    assert!(
        err.contains("M002") || err.contains("not exported"),
        "should be not-exported error: {}",
        err
...
fn wildcard_import_skips_private_items() {
    ...
    let err = crate::common::check_err_with_files("use mixed\nlet p = Private(x: 1)\n", files);
    assert!(
...
fn module_not_found() {
    ...
    let err = crate::common::check_err_with_files("use nonexistent/module { Foo }\n", files);
    assert!(
...
fn name_not_in_module() {
    ...
    let err = crate::common::check_err_with_files("use stuff { DoesNotExist }\n", files);
    assert!(
        err.contains("M002") || err.contains("not exported"),
...
fn circular_import_detected() {
    ...
    let err = crate::common::check_err_with_files("use a { X }\nlet x = X(val: 1)\n", files);
    assert!(
...
fn namespace_import_call_function() {
    ...
    crate::common::check_ok_with_files("use utils as u\nlet x = u.double(n: 5)\n", files);
}
...
fn namespace_import_access_variable() {
    let mut files = HashMap::new();
    files.insert("config".to_string(), "pub let VERSION = 42\n".to_string());
    crate::common::check_ok_with_files("use config as cfg\nlet v: Int = cfg.VERSION\n", files);
}
...
fn namespace_member_not_exported() {
    let mut files = HashMap::new();
    files.insert("stuff".to_string(), "pub class Foo\n  x: Int\n".to_string());
    let err = crate::common::check_err_with_files("use stuff as s\nlet b = s.Bar(x: 1)\n", files);
    assert!(
        err.contains("M004")
...
fn selective_with_alias_is_error() {
    let mut files = HashMap::new();
    files.insert("stuff".to_string(), "pub class Foo\n  x: Int\n".to_string());
    let err = crate::common::check_err_with_files("use stuff { Foo } as s\n", files);
    assert!(
...
fn pub_use_reexports_class() {
    ...
    crate::common::check_ok_with_files("use models { User }\nlet u = User(name: \"Jo\")\n", files);
}
...
fn pub_use_reexports_function() {
    ...
    crate::common::check_ok_with_files("use math { double }\nlet x = double(n: 5)\n", files);
}
...
fn pub_use_reexports_enum() {
    ...
    crate::common::check_ok_with_files("use types { Color }\nlet c = Color.Red\n", files);
}
...
fn pub_use_only_reexports_pub_items_from_source() {
    ...
    let err = crate::common::check_err_with_files("use facade { Private }\n", files);
    assert!(
        err.contains("M002") || err.contains("not exported"),
...
fn non_pub_use_does_not_reexport() {
    ...
    let err = crate::common::check_err_with_files("use facade { User }\n", files);
    assert!(
        err.contains("M002") || err.contains("not exported"),
...
fn chained_reexports() {
    ...
    crate::common::check_ok_with_files("use top { Widget }\nlet w = Widget(id: 42)\n", files);
}
...
fn pub_use_accessible_via_namespace() {
    ...
    crate::common::check_ok_with_files("use models as m\nlet u = m.User(name: \"Jo\")\n", files);
}
