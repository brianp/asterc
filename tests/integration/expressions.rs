...
fn arithmetic_expression() {
    crate::common::check_ok("let x = 1 + 2 * 3");
}

#[test]
fn float_promotion() {
    crate::common::check_ok("let x = 1 + 2.0");
}

#[test]
fn string_concat() {
    crate::common::check_ok("let x = \"hello\" + \" world\"");
}
...
fn logical_expression() {
    crate::common::check_ok("let x = true and false or true");
}

#[test]
fn unary_neg() {
    crate::common::check_ok("let x = -5");
}

#[test]
fn unary_not() {
    crate::common::check_ok("let x = not true");
}

#[test]
fn nested_precedence() {
    crate::common::check_ok("let x = (1 + 2) * 3 - 4 / 2");
}

#[test]
fn grouped_expression() {
    crate::common::check_ok("let x = (1 + 2) * (3 + 4)");
}

#[test]
fn if_with_comparison() {
    crate::common::check_ok("if 1 < 2\n  3\n");
}

#[test]
fn function_with_binary_ops() {
    crate::common::check_ok("def add(a: Int, b: Int) -> Int\n  a + b\n");
}

#[test]
fn function_with_return() {
    crate::common::check_ok("def f() -> Int\n  return 42\n");
}

#[test]
fn string_sub_type_error() {
    let err = crate::common::check_err("let x = \"a\" - \"b\"");
    assert!(
...
fn int_and_bool_error() {
    let err = crate::common::check_err("let x = 1 + true");
    assert!(
        err.contains("Cannot apply")
            || err.contains("incompatible types")
            || err.contains("used outside"),
...
#[test]
fn not_int_error() {
    let err = crate::common::check_err("let x = not 5");
    assert!(err.contains("Cannot apply 'not'"));
}

#[test]
fn comparison_type_mismatch() {
    let err = crate::common::check_err("let x = 1 == \"a\"");
...
#[test]
fn while_loop() {
    crate::common::check_ok("let x = true\nwhile x\n  log(message: \"hi\")\n");
}

#[test]
fn while_non_bool_error() {
    let err = crate::common::check_err("while 1\n  1\n");
    assert!(
...
#[test]
fn for_loop() {
    crate::common::check_ok("let items = [\"hi\", \"there\"]\nfor x in items\n  log(message: x)\n");
}

#[test]
fn if_elif_else() {
    crate::common::check_ok("if true\n  1\nelif false\n  2\nelse\n  3\n");
}
...
#[test]
fn assignment() {
    crate::common::check_ok("let x = 1\nx = 2\n");
}

#[test]
fn assignment_type_mismatch() {
    let err = crate::common::check_err("let x = 1\nx = \"hello\"\n");
    assert!(err.contains("mismatch") || err.contains("Mismatch"));
}

#[test]
fn break_in_while() {
    crate::common::check_ok("while true\n  break\n");
}
...
#[test]
fn log_builtin() {
    crate::common::check_ok("log(message: \"hello\")");
}
