...
fn lex_error_is_diagnostic() {
    ...
    let diag = crate::common::check_lex_err_diagnostic(src);
    assert_eq!(diag.severity, ast::Severity::Error);
}
...
let a: Int = \"hello\"
let b: Int = \"world\"";
    let diags = crate::common::check_all_errors(src);
    assert!(
        diags.len() >= 2,
...
fn parser_recovery_reports_multiple_errors() {
    ...
    let src = "let x = )\nlet y = 42";
    let result = crate::common::parse_with_recovery(src);
    assert!(
...
fn lowercase_builtin_type_rejected() {
    for (wrong, right) in [
        ...
        let err = crate::common::check_parse_err(&src);
        assert!(
